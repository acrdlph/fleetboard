# orchestr for iOS — engineering plan

Status: **plan**. Written against `/Users/achill/Downloads/orchestr/orchestr.py` at HEAD
(2302 lines). The client is built for the **versioned, op-addressed, SSE-streamed API** the
backend ships first — `Orchestr-Api: 1.0`, served under `/api/v1/`. There is no legacy mode;
see §5.6.
...
|---|---|---|---|
| `Orchestr` | iOS app | all `OrchestrKit` products | the app |
| `OrchestrWidgets` | Widget extension | `OrchestrCore`, `OrchestrPersistence` **only** | home/lock-screen attention count, Live Activity UI |
| `OrchestrNotificationService` | Notification Service extension | `OrchestrCore`, `OrchestrPersistence`, `OrchestrAPI` | enrich a push body from `/api/events/<id>` |
| `OrchestrKit` | local SwiftPM package | — | everything testable without a simulator |
| `SnapshotTests` | app-hosted XCTest | `OrchestrUI`, `OrchestrTestSupport` | real `UIWindow` rendering (§9.3) |
| `UITests` | XCUITest | — | the scroll-hold behaviour + accessibility audits |

**The extensions deliberately do not link `OrchestrStore`.** That is a build-time guarantee
...
```
orchestr/
├── orchestr.py · index.html · map.html · limits.html · guide.html · start.sh · tests/
├── docs/mobile/
...
└── ios/
    ...
    ├── App/                              # target: Orchestr — composition + SwiftUI only
    │   ├── OrchestrApp.swift
    ...
    │   │   └── DeepLinkAdapter.swift     # calls OrchestrCore.DeepLink.parse
    │   ├── Resources/
    │   │   ├── Assets.xcassets · PrivacyInfo.xcprivacy
    │   └── Orchestr.entitlements
    ├── Widgets/
    │   ├── OrchestrWidgetBundle.swift
    ...
    │   └── OrchestrWidgets.entitlements
    ├── NotificationService/
    │   ├── NotificationService.swift
    │   └── OrchestrNotificationService.entitlements
    ├── SnapshotTests/
    ...
    └── Packages/OrchestrKit/
        ├── Package.swift
        ├── Sources/
        │   ├── OrchestrCore/              # nonisolated · pure · zero I/O
        │   │   ├── Model/
        ...
        │   ├── OrchestrPersistence/       # nonisolated · actors over the filesystem
        │   │   ├── FileStore.swift · KeychainStore.swift · Defaults.swift
        │   │   └── AppGroup.swift
        │   ├── OrchestrAPI/               # nonisolated · actor client + SSE
        │   │   ├── Transport/
        ...
        │   │   ├── APIClient.swift · Endpoint.swift · OrchestrError.swift
        ...
        │   ├── OrchestrStore/             # @MainActor by default · behaviour only
        ...
        │   ├── OrchestrUI/                # @MainActor by default
        ...
        │   └── OrchestrTestSupport/       # A PRODUCT — the app-hosted tests import it
        ...
        └── Tests/
            ├── OrchestrCoreTests/ · OrchestrPersistenceTests/ · OrchestrAPITests/
            ├── OrchestrStoreTests/ · OrchestrUIKitTests/ · OrchestrIntegrationTests/
```
...
let package = Package(
    ...
    products: [
        .library(name: "OrchestrCore",        targets: ["OrchestrCore"]),
        .library(name: "OrchestrPersistence", targets: ["OrchestrPersistence"]),
        .library(name: "OrchestrAPI",         targets: ["OrchestrAPI"]),
        .library(name: "OrchestrStore",       targets: ["OrchestrStore"]),
        .library(name: "OrchestrUI",          targets: ["OrchestrUI"]),
        // A PRODUCT: SnapshotTests and UITests live outside the package and cannot
        // import a non-product target.
        .library(name: "OrchestrTestSupport",  targets: ["OrchestrTestSupport"]),
    ],
    dependencies: [],                        // zero, matching the project's identity
    targets: [
        .target(name: "OrchestrCore",        swiftSettings: nonisolatedDefault),
        .target(name: "OrchestrPersistence", dependencies: ["OrchestrCore"],
                swiftSettings: nonisolatedDefault),
        .target(name: "OrchestrAPI",         dependencies: ["OrchestrCore"],
                swiftSettings: nonisolatedDefault),
        .target(name: "OrchestrStore",       dependencies: ["OrchestrAPI", "OrchestrPersistence"],
                swiftSettings: mainActorDefault),
        .target(name: "OrchestrUI",          dependencies: ["OrchestrStore"],
                swiftSettings: mainActorDefault),
        .target(name: "OrchestrTestSupport",
                dependencies: ["OrchestrAPI", "OrchestrPersistence"],
        ...
        .testTarget(name: "OrchestrCoreTests",
                    dependencies: ["OrchestrCore", "OrchestrTestSupport"],
                    swiftSettings: nonisolatedDefault),
        .testTarget(name: "OrchestrPersistenceTests",
                    dependencies: ["OrchestrPersistence", "OrchestrTestSupport"],
                    swiftSettings: nonisolatedDefault),
        .testTarget(name: "OrchestrAPITests",
                    dependencies: ["OrchestrAPI", "OrchestrTestSupport"],
                    swiftSettings: nonisolatedDefault),
        ...
        .testTarget(name: "OrchestrStoreTests",
                    dependencies: ["OrchestrStore", "OrchestrTestSupport"],
                    swiftSettings: mainActorDefault),
        .testTarget(name: "OrchestrUIKitTests",
                    dependencies: ["OrchestrUI", "OrchestrTestSupport"],
                    swiftSettings: mainActorDefault),
        .testTarget(name: "OrchestrIntegrationTests",
                    dependencies: ["OrchestrAPI", "OrchestrTestSupport"],
                    swiftSettings: nonisolatedDefault),
    ]
)
...
<string>orchestr scans the pairing code shown on your Mac.</string>
<key>NSLocalNetworkUsageDescription</key>
<string>orchestr connects to your Mac over your Tailscale network.</string>
<key>NSSupportsLiveActivities</key><true/>
<key>NSSupportsLiveActivitiesFrequentUpdates</key><true/>
<key>BGTaskSchedulerPermittedIdentifiers</key>
<array><string>sh.orchestr.refresh</string></array>
<key>CFBundleURLTypes</key>
<array><dict>
  <key>CFBundleURLName</key><string>sh.orchestr.pair</string>
  <key>CFBundleURLSchemes</key><array><string>orc</string><string>orchestr</string></array>
</dict></array>
```
...
<array><string>group.sh.orchestr</string></array>
<key>keychain-access-groups</key>
<array><string>$(AppIdentifierPrefix)sh.orchestr.shared</string></array>
```
...
```
 ...
 HTTP ─▶│ URLSessionTransport                 (structs from OrchestrCore)  │
        └──────────────────────────────┬───────────────────────────────────┘
...
```swift
// OrchestrAPI/Transport/Transport.swift
...
```swift
// OrchestrAPI/Transport/SSETransport.swift
//
...
private func perform<T: Decodable & Sendable>(_ e: Endpoint, as _: T.Type) async throws -> T {
    ...
    req.setValue("orchestr-ios/\(Bundle.shortVersion)", forHTTPHeaderField: "X-Orchestr-Client")
    req.setValue("Bearer \(context.token)", forHTTPHeaderField: "Authorization")
    if let opID = e.opID { req.setValue(opID, forHTTPHeaderField: "X-Orchestr-Op-Id") }
    if let body = try e.jsonBody(encoder: encoder) {
        // do_POST reads exactly Content-Length bytes (orchestr.py:2249). URLSession sends
    ...
    do { res = try await transport.send(req) }
        ...
        if Task.isCancelled { throw OrchestrError.cancelled }
        if let u = error as? URLError, u.code == .cancelled { throw OrchestrError.cancelled }
        throw OrchestrError.transport(TransportFailure.classify(error))
    }

    switch res.status {
    case 200, 202:  break
    case 401:       throw OrchestrError.unauthorized
    case 403:       throw OrchestrError.forbidden(reason: res.jsonError ?? "forbidden")
    case 409:       throw OrchestrError.conflict(res.jsonError ?? "already in flight")
    case 429:       throw OrchestrError.rateLimited(retryAfter: res.retryAfterSeconds)
    default:
        // send_error(404) writes an HTML body. Never assume JSON off the happy path.
        throw OrchestrError.httpStatus(
            code: res.status,
    ...
    catch let d as DecodingError { throw OrchestrError.decoding(.from(d, endpoint: e.path)) }
    if let f = decoded as? EnvelopeBearing, f.isFailure { throw OrchestrError.from(f, endpoint: e) }
    return decoded
}
```
...
/// (`out.get("ok", True)`, tests/test_orchestr.py:558) and `start_dispatch`'s success path
...
`read_chat` matches `account=([^&]+)` with no `urllib.parse.unquote` (orchestr.py:2219)
...
```swift
// OrchestrCore/Format/AccountLabel.swift
...
```swift
// OrchestrStore/ActionGateway.swift   (@MainActor by module default)
@Observable
public final class ActionGateway {
    public func arm(_ kind: Intent.Kind, _ target: TargetRef,
                    payload: IntentPayload = .none) async -> Result<Intent, OrchestrError>
    public func confirm(_ id: IntentID) async -> Result<Intent, OrchestrError>
    public func cancel(_ id: IntentID)  async -> Result<Intent, OrchestrError>
    ...
    public func send(text: String, to sid: SessionID) async -> Result<Intent, OrchestrError> {
        // send_to_process collapses all newlines to single spaces (orchestr.py:1344).
        let normalized = text
        ...
        // orchestr.py:1359 lacks the `--` sentinel. Refuse locally with a fixable reason
        ...
        if normalized.hasPrefix("-") && !caps.has(.dashSentinel) {
            return .failure(.clientPrecondition(
                ...
                + "prefix it with a word, or update orchestr"))
        }
...
Two layers, deliberately: **DTOs** in `OrchestrAPI/DTO/` decode the wire exactly as the
server writes it; **domain types** in `OrchestrCore/Model/` are what the app renders. The
...
Every domain type is `Sendable`, `Equatable`, and a `struct`. Nothing in `OrchestrCore`
touches Foundation networking, the filesystem, or `Date()`.

## 3.1 Identity

```swift
// OrchestrCore/Identity/
...
> `discover_worktrees` (orchestr.py:120-138) dedupes by absolute path, so two roots each
...
```swift
// OrchestrCore/Model/SessionStatus.swift
...
```swift
// OrchestrCore/Model/FleetSnapshot.swift
...
public struct Counts: Sendable, Equatable, Codable {
    ...
    /// already excluded server-side (orchestr.py:798-801).
...
```swift
// OrchestrCore/Model/Worktree.swift
public struct Worktree: Sendable, Equatable, Identifiable {
    ...
    /// non-empty (orchestr.py:776-781). Its presence flips ✓ finish → ✕ close.
...
```swift
// OrchestrCore/Model/Session.swift
public struct Session: Sendable, Equatable, Identifiable {
    ...
    public var account: String                    // orchestr LABEL ("main"), not cclimits slug
    ...
    /// account (orchestr.py:664). false = a freshness-order GUESS. The UI must render a
...
public struct SessionLimit: Sendable, Equatable {
    ...
    /// (orchestr.py:728-732) fires when the CLI wrote "you've hit your session limit" but
...
```swift
// OrchestrCore/Model/LimitsSnapshot.swift
...
public struct Account: Sendable, Equatable, Identifiable {
    ...
    public var label: String?                     // fb_label — orchestr's, injected. THE key.
...
```swift
// OrchestrCore/Model/Intent.swift
...
```swift
// OrchestrCore/Model/ResumeSchedule.swift
...
// OrchestrCore/Model/ChatMessage.swift
...
// OrchestrCore/Model/DispatchLogEntry.swift
...
```swift
// OrchestrCore/Model/StateFrame.swift
...
// OrchestrCore/Model/ServerMeta.swift
...
// OrchestrCore/Model/Freshness.swift
...
```swift
// OrchestrAPI/DTO/SessionDTO.swift
...
```swift
// OrchestrCore/Format/TextTruncation.swift
public enum Trunc {
...
```swift
// OrchestrCore/Format/RelativeTime.swift
...
// OrchestrCore/Format/ETime.swift
public enum ETime {
...
// OrchestrCore/Format/ClockLabel.swift
...
`OrchestrCoreTests` exercises every branch of every one of these against the JS source
...
|---|---|---|
| `OrchestrCore` | **nonisolated** | must be callable from a widget timeline provider, an NSE, and a `TestClock` actor |
| `OrchestrPersistence` | **nonisolated** | `FileStore` is an `actor`; it decodes `Persisted/` types from off-MainActor |
| `OrchestrAPI` | **nonisolated** | `APIClient` is an `actor`; the SSE delegate is `@unchecked Sendable` |
| `OrchestrStore` | **`MainActor`** | `@Observable` is not `Sendable`; stores are main-thread by construction |
| `OrchestrUI` | **`MainActor`** | SwiftUI |
| `OrchestrTestSupport` | **nonisolated** | must be importable from both kinds of test target |
...
```swift
// OrchestrCore/Time/AppClock.swift — nonisolated, ~40 lines
...
```swift
// OrchestrTestSupport/TestClock.swift — ~80 lines. Never touches the system clock.
...
`OrchestrCore/Persisted/` and every store-over-the-filesystem lives in
`OrchestrPersistence`, both nonisolated. `OrchestrPersistenceTests` decodes each persisted
...
| 2 | `@State private var model = AppModel()` in a nonisolated `App` | `@main @MainActor struct OrchestrApp: App` |
...
| 8 | widget `TimelineProvider` / NSE run outside MainActor and cannot touch stores | enforced by the **package graph** — neither extension target links `OrchestrStore` |
| 9 | `nonisolated(unsafe)` as a silencer | banned outside `OrchestrTestSupport`; same lint |
...
#!/bin/sh
set -eu
SRC=ios/Packages/OrchestrKit/Sources
FAIL=0
check() { if grep -rn "$1" $2 --include=*.swift | grep -v "$3"; then
            echo "  ✗ $4"; FAIL=1; fi; }

check 'Task\.detached'        "$SRC/OrchestrStore $SRC/OrchestrUI ios/App" 'XXNOMATCHXX' \
...
check 'nonisolated(unsafe)'   "$SRC" 'OrchestrTestSupport' \
      'nonisolated(unsafe) is banned outside OrchestrTestSupport'
check 'AnyView'               "$SRC/OrchestrUI ios/App/Board" 'XXNOMATCHXX' \
...
check 'nonisolated func .*async' "$SRC/OrchestrCore $SRC/OrchestrAPI" '@concurrent' \
...
```swift
// OrchestrStore/FleetStore.swift
@Observable
...
```swift
// OrchestrCore/Rules/ReorderHold.swift — nonisolated, no SwiftUI import
...
```swift
// OrchestrStore/LiveUpdateEngine.swift
@Observable
...
```swift
// OrchestrStore/ConnectionStore.swift (excerpt)
...
```swift
// OrchestrCore/Time/ServerClock.swift — nonisolated value, single owner
...
```swift
// OrchestrStore/FreshnessStore.swift
...
guard meta.contract >= 2 else {
    throw OrchestrError.serverTooOld(found: meta.contract, required: 2)
}
```

One blocking screen: *"starbase is running orchestr 1.x. This app needs 2.0 or newer — pull
...
the user restarting an old `orchestr.py` — is better surfaced than smoothed over.
...
(`cached_limits` subprocess timeout, orchestr.py:1035) against a 2.5 s launch budget.
...
(orchestr.py:701-704) — so a phone polling only `/api/state` sees limit-stuck agents as
...
One `BGAppRefreshTask` (`sh.orchestr.refresh`, `earliestBeginDate` +15 min) refreshes the
...
```swift
// OrchestrCore/Rules/ErrnoCause.swift — pure, unit-tested with zero I/O
...
| 3 | TCP connects, then refused/reset | **orchestr isn't running** | the Mac is reachable — run `./start.sh` on starbase |
...
| 5 | `/api/health` `contract` < 2, or 404 on a known route | **starbase is running orchestr 1.x** | pull and run `./start.sh` |
...
```swift
// OrchestrStore/PushCoordinator.swift  (@MainActor; testable — no UIKit in here)
@Observable
...
```swift
// OrchestrCore/Rules/DeepLink.swift
...
/// orchestr://worktree?name=<pct-encoded>   ·  orchestr://session?sid=<uuid>
/// orc://p?h=<host>&c=<code>&f=<pin>        ·  orchestr://mission?text=<pct-encoded>
///
/// A QUERY PARAMETER, not a path segment: worktree names and paths contain "/".
public func parse(_ url: URL) -> Route?
```

Routing lives in `OrchestrCore` and not in the App target, because the App target has no
unit test home and this is pure string handling with a security-relevant failure mode — a
malformed `orchestr://` URL landing on the wrong worktree, in an app whose actions type
...
```swift
// OrchestrCore/Persisted/PairPayload.swift
public struct PairPayload: Sendable, Equatable, Codable {
    ...
    public init?(url: URL)        // accepts orc://p and orchestr://pair
}
```
...
```swift
// OrchestrPersistence/KeychainStore.swift  (nonisolated; actor-safe)
public enum TokenScope: String, Sendable { case read, act }

public enum KeychainStore {
    public static let service = "sh.orchestr.token"
    ...
    public static let accessGroup = "$(AppIdentifierPrefix)sh.orchestr.shared"
...
```swift
// OrchestrAPI/Transport/PinnedTrustDelegate.swift
...
```swift
// OrchestrPersistence/FileStore.swift
...
Target `OrchestrWidgets` links `OrchestrCore` and `OrchestrPersistence` **only** — a
...
| `systemMedium` | the same plus up to three card names with their status glyph and worktree, tap-targeted to `orchestr://worktree?name=…` |
...
```swift
// OrchestrTestSupport/MockTransport.swift
...
@Test func nullLimitRenders() throws {
    // orchestr.py:728-732 — the transcript-regex fallback sets all four to null.
...
@Test func handedOffIsNotAttention() throws { … }        // orchestr.py:798-801
...
func mutationsDegradeCleanly(_ m: Mutation) throws {
    ...
    // throw a typed OrchestrError.decoding — never trap, never crash.
...
The obvious drift guard — running the Swift decoder against `orchestr.py --demo` — tests
...
`card()` (orchestr.py:974-980) assigns `pid`/`pid_certain` **only to non-`ended` sessions**
while real state sets both unconditionally — so the expected value depends on the mood of
the fleet at capture time. Also `closeout_sent` is a **card**-level key (orchestr.py:779),
...
```swift
// OrchestrUI/Snapshot/ViewSnapshotter.swift — app-hosted, ~120 lines, zero dependencies
@MainActor
...
## 9.5 Integration against `python3 orchestr.py --demo`
...
public actor DemoServer {
    ...
    /// developer's machine: load_config reads HERE/orchestr.config.json, so a locally
    ...
    public static func start(scriptPath: URL) async throws -> Handle {
        ...
        try sandbox.write("orchestr.config.json", Fixtures.demoConfig)   // roots/homes → sandbox
        ...
        // `--port 0` is NOT usable: load_config does `if args.port:` (orchestr.py:97) and 0
...
CI additionally fails the job if the executed test count for `OrchestrIntegrationTests`
...
steps:
  ...
  - run: swift build -c release --package-path ios/Packages/OrchestrKit
  - run: swift test            --package-path ios/Packages/OrchestrKit
  ...
  - run: xcodebuild build -scheme Orchestr -destination 'generic/platform=iOS'
  - run: |
      SIM=$(xcrun simctl list devices available -j | ios/Tools/pick-sim.sh)   # resolved,
      xcodebuild test -scheme Orchestr -testPlan Snapshot -destination "$SIM" # not hardcoded
      xcodebuild test -scheme Orchestr -testPlan UI       -destination "$SIM"
```
...
| thing | value |
|---|---|
| App bundle ID | `sh.orchestr.app` |
| Widget extension | `sh.orchestr.app.widgets` |
| Notification service extension | `sh.orchestr.app.notifications` |
| App Group | `group.sh.orchestr` |
| Keychain access group | `$(AppIdentifierPrefix)sh.orchestr.shared` |
| APNs topic (alerts) | `sh.orchestr.app` |
| APNs topic (Live Activity) | `sh.orchestr.app.push-type.liveactivity` |
| Live Activity attributes type | `LimitResetAttributes` |
| BGTask identifier | `sh.orchestr.refresh` |
| URL schemes | `orc`, `orchestr` |

## 10.3 Signing

```
Configs/Shared.xcconfig       # everything non-secret, committed
  PRODUCT_BUNDLE_IDENTIFIER = sh.orchestr.app
...
Configs/Release.xcconfig
  ...
  PROVISIONING_PROFILE_SPECIFIER = orchestr-appstore

Configs/Signing.xcconfig      # GITIGNORED — the only file holding the team
  DEVELOPMENT_TEAM = XXXXXXXXXX
```

`Orchestr.entitlements` references `$(APS_ENVIRONMENT)` so one plist serves both
...
# Tools/release.sh
set -euo pipefail
xcodebuild -scheme Orchestr -configuration Release \
           -destination 'generic/platform=iOS' \
           -archivePath build/Orchestr.xcarchive archive
xcodebuild -exportArchive -archivePath build/Orchestr.xcarchive \
           -exportOptionsPlist Configs/ExportOptions.plist \
           -exportPath build/export
xcrun altool --upload-app -f build/export/Orchestr.ipa -t ios \
...
|---|---|
| the extensions never link `OrchestrStore` | the package graph (§1.2) |
...
orchestr, in the iOS battery list. Two supported modes: always-on tunnel (the NSE can
...
1. **NE-tunnel errno behaviour.** Against a real tailnet: Mac asleep · Mac off the tailnet ·
   orchestr stopped. Confirm `ECONNREFUSED` propagates (expected: yes) and whether
...
| **4** | **`limit` status without an inbound `/api/limits`** | otherwise a phone that never opens the limits view mis-triages limit-stuck agents into the loudest state in the app | `FRESHNESS.md §6` · orchestr.py:701-704 |
...
| **9** | **`parse_qs` the query string** (fixes the never-decoded `account` param) | any label needing percent-encoding is unreachable today | orchestr.py:2219 |
| **10** | `--` sentinel before every `send-keys -l` / `set-buffer` argument; `_proven_in_transcript` receipt on send | **verified on tmux 3.6a**: `send-keys -t T -l "-n foo"` exits 1, "unknown flag -n". And `ok:true` currently means "keys accepted", not "agent received" | orchestr.py:1359, :1717 |
| **11** | Reject a past `due_at` on resume rather than clamping to `now+5` | the clamp is a desktop convenience that is wrong for a client that can be offline for hours | orchestr.py:1966 |
| **12** | **Resolve the card-key collision** — path-derived key, or refuse a fleet with duplicate basenames | `discover_worktrees` dedupes by path while `cards` is keyed by name → one card silently lost, and `wt:<name>` mutations are ambiguous | orchestr.py:120-138 |
| **13** | `try/except` around `_run_dispatch` and `do_POST`; cap and validate `Content-Length` | a bad payload strands a job at `done:false` forever; a **negative** `Content-Length` becomes `read(-1)` = read-to-EOF = hang | orchestr.py:2249 |
| **14** | Atomic `save_resumes` (tmp + `os.replace`); locks on `_cache` / `_closeouts` | `write_text` truncate-then-write plus `except OSError: pass` silently loses every armed schedule | orchestr.py:1885 |
| **15** | Cache `read_dispatch_log` behind a 5 s TTL; tail-read instead of `read_text()` on the whole file | it reads and `json.loads`-parses the **entire** unbounded log plus a `tmux list-sessions` subprocess on every call, for 25 entries — 38 KB, larger than the board | orchestr.py:1613-1633 |
...
| **17** | `--port-file PATH` (3 lines) | `--port 0` is silently ignored (`if args.port:` — 0 is falsy) and binds 4242, colliding with the user's real board | orchestr.py:97 |
...
| **19** | `do_HEAD` | §5.10's tunnel warm-up; currently 501 | orchestr.py |
