...
| **Next** | step 7, second half — device pairing (a code and a QR, rather than a token carried by hand) and the tailnet bind. **Authentication exists** as of ADR 0014: per-device tokens stored hashed, one check no route can get past, loopback trusted, an audit log of every mutation, a failure budget, and a bind that refuses to expose the fleet with nobody registered |
| **Tests** | 577 · characterization 5,656 cases (18 sections) |
...
| scopes (`read`/`act`/`admin`) are designed and not built | one token grants everything. API.md §2.2 splits them and issues two per device; a phone that can read but not act is not the product, so the split buys nothing yet — and a half-built ladder is worse than an honest absence, because it invites the belief that a `read` token is safe to hand out | API.md §2.2, ADR 0014 |
| tokens are carried to the phone by hand | `--add-device` prints one once. Pairing (API.md §3: a 120 s window, a six-digit code, a QR) needs a second unauthenticated route with its own attempt limit — it inherits the failure budget that is already here, and it wants a phone to walk through it | ADR 0014 |
