...
| **Next** | step 6 (hooks) and step 8 (APNs). **Step 7 is done**: authentication (ADR 0014) and now pairing and the bind (ADR 0015) — a phone scans a QR carrying a 120-second single-use ticket, exchanges it for a token, and is listed and revocable from the board. The tailnet address is detected rather than typed, `0.0.0.0` needs a flag that says what it is, and no bind beyond loopback succeeds with no device registered |
| **Tests** | 700 · characterization 5,656 cases (18 sections) · the QR encoder additionally checked against Apple's `CIQRCodeGenerator` module for module and decoded back by Vision (`tests/qr_ref.py`) |
